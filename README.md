...
DeepSeek V4 API 采用峰谷定价：**空闲时段价格为高峰时段价格的一半，高峰时段为北京时间每日 9:00–12:00、14:00–18:00**，其余时间为低谷（空闲）时段。

自北京时间 **2026 年 8 月 23 日（周日）00:00** 起，峰谷计费规则调整：**周末（周六、周日）全天不再区分峰谷时段，统一按照低谷时段价格收取调用费用**；工作日照常按上述高峰/低谷时段划分。
